...
// meanSpeed возвращает значение средней скорости(в км/ч) движения во время тренировки.
//
// Параметры:
//
// action int — количество совершенных действий(число шагов при ходьбе и беге, либо гребков при плавании).
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	heightM := height / 100
	// fmt.Println(action, duration, weight, heightM, speedMS, speed, math.Pow(speedMS, 2))
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(speedMS, 2)/heightM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
...
// swimmingMeanSpeed возвращает среднюю скорость(в км/ч) при плавании.
//
// Параметры:
//
